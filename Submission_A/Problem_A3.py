...
def solution_A3():
    ...
    train_generator = train_datagen.flow_from_directory(
        train_dir,  # directory file
        target_size=(150, 150),  # resize
        batch_size=128,
        class_mode='binary')  # pakai binary karena losss binary crossentropy

    val_generator = val_datagen.flow_from_directory(
        validation_dir,  # directory file
        target_size=(150, 150),  # resize
        batch_size=128,
        class_mode='binary')  # pakai binary karena losss binary crossentropy
    ...
    model.compile(optimizer=RMSprop(learning_rate=0.0001),
                  loss='binary_crossentropy', #pakai binary soalnya outputnya ada dua
                  metrics=['acc'])
    model.fit(
        train_generator,
